...
namespace UnpackMiColorFace.Helpers
{
	public class BmpHelper
	{
		...
		private static byte[] headerARgb32_Rev = new byte[122]
		{
			66, 77, 246, 220, 2, 0, 0, 0, 0, 0,
			122, 0, 0, 0, 108, 0, 0, 0, 3, 1,
			0, 0, 181, 0, 0, 0, 1, 0, 32, 0,
		...
		public static byte[] ConvertToBmpGTR(byte[] data, int width, int height, int type, byte[] clut = null)
		{
			...
			data = FlipImageData(data, width, height, type);
			data = array.Concat(data).ToArray();
			int val2 = data.Length;
			data.SetDWord(2, (uint)val2);
			if (type == 1)
			{
				data.SetByteArray(54, clut);
			}
		...
		internal static byte[] ConvertToBmpGTRv2(byte[] data, int width, int height, int type)
		{
			...
			else
			{
				...
				if (type != 4)
				{
					data = AlignRowData(data, width, height, type, out dstWidth2);
				}
				else
				{
					dstWidth2 = width;
			...
			if (type == 1)
			{
				data.SetDWord(46, (uint)num2);
				data.SetDWord(50, (uint)num2);
			}
			else
			{
				data.SetDWord(34, (uint)num);
		...
		private static byte[] AlignRowData(byte[] src, int width, int height, int type, out int dstWidth)
		{
			...
			if (num == num.GetDWordAligned() && num == src.Length)
			{
				dstWidth = width;
				return src;
			}
			uint dWordAligned = num.GetDWordAligned();
			dstWidth = (int)(dWordAligned / type);
			byte[] array = new byte[dWordAligned * height];
			uint num2 = 0u;
			for (int i = 0; i < height; i++)
			{
				array.SetByteArray((int)(i * dWordAligned), src.GetByteArray(num2, num));
				num2 += num;
			}
			return array;
		}
		...
		private static byte[] FlipImageData(byte[] src, int width, int height, int type)
		{
			...
			if (num * height > src.Length)
			{
				throw new ApplicationException($"image processing err, image len: {src.Length}, but expected: {num * height}");
			}
