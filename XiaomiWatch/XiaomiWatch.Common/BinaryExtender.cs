...
namespace XiaomiWatch.Common
{
	...
	{
		public static string TrimSingleTrailingQuote(this string str)
		{
			str = str.TrimEnd();
		...
		public static byte[] ReverseWord(this byte[] data)
		{
		...
		public static byte[] To32Argb(this byte[] data, int cprType)
		{
			...
			case 1:
			{
				int num5 = data.Length / 3 * 4;
				array = new byte[num5];
				int num6 = 0;
				for (int k = 0; k < num5 - 4; k += 4)
				{
					byte b = data[num6];
					byte b2 = data[num6 + 1];
					byte b3 = data[num6 + 2];
		...
		public static byte[] Rgb565AlphaToRGBA(this byte[] data)
		{
			byte[] array = data;
			...
			for (int i = 0; i < num - 4; i += 4)
			{
				byte b = (byte)(data[num2 + 1] >> 3);
				byte num3 = (byte)(((data[num2 + 1] & 7) << 3) | (data[num2] >> 5));
				byte b2 = (byte)(data[num2] & 0x1F);
				byte b3 = data[num2 + 2];
				byte b4 = (byte)(b * 527 + 23 >> 6);
				byte b5 = (byte)(num3 * 259 + 33 >> 6);
				byte b6 = (byte)(b2 * 527 + 23 >> 6);
				array[i] = b6;
				array[i + 1] = b5;
				array[i + 2] = b4;
				array[i + 3] = b3;
		...
		public static uint GetDWord(this byte[] data, int offset = 0, uint bigEndian = 0u)
		{
		...
		public static void SetDWord(this byte[] data, int offset = 0, uint val = 0u, uint bigEndian = 0u)
		{
			data.SetDWord((uint)offset, val, bigEndian);
		}

		public static void SetDWord(this byte[] data, int offset = 0, int val = 0, uint bigEndian = 0u)
		{
			data.SetDWord((uint)offset, (uint)val, bigEndian);
		}
		...
		public static string GetUtf8String(this byte[] data, uint offset = 0u)
		{
			uint num;
			for (num = 0u; data[offset + num] != 0; num++)
			{
		...
		public static string[] GetUTF8StringArray(this byte[] data, uint offset = 0u, uint strlen = 0u, uint bigEndian = 0u)
		{
			List<string> list = new List<string>();
			for (uint num = 0u; num < strlen; num++)
			{
				for (; data[offset + num] != 0; num++)
				{
				}
				byte[] array = new byte[num];
				Array.Copy(data, offset, array, 0L, num);
				...
				list.Add(Encoding.UTF8.GetString(array));
			}
			return list.ToArray();
		}
		...
		public static ushort GetWord(this byte[] data, int offset = 0, int bigEndian = 0)
		{
			return data.GetWord((uint)offset, (uint)bigEndian);
		}
		...
		public static void SetWord(this byte[] data, int offset = 0, ushort val = 0, uint bigEndian = 0u)
		{
			data.SetWord((uint)offset, val, bigEndian);
		}

		public static void SetWord(this byte[] data, int offset = 0, int val = 0, uint bigEndian = 0u)
		{
			data.SetWord((uint)offset, (ushort)val, bigEndian);
		}
		...
		public static byte GetByte(this byte[] data, int offset = 0)
		{
			return data[offset];
		}
		...
		public static byte[] ShiftLeft(this byte[] data, int bits)
		{
			int num = 0;
			for (int i = 0; i < data.Length; i++)
			{
				byte b = data[i];
				data[i] = (byte)((b << bits) | num);
				num = b >> 8 - bits;
			}
			if (num > 0)
			{
				data = data.AppendZero();
				data[data.Length - 1] = (byte)num;
			}
			return data;
		}

		public static byte[] GetByteArray(this byte[] src, int offset = 0, int len = 1)
		{
			return src.GetByteArray((uint)offset, (uint)len);
		}

		public static byte[] GetByteArray(this byte[] src, uint offset = 0u, int len = 1)
		{
			return src.GetByteArray(offset, (uint)len);
		}

		public static byte[] GetByteArray(this byte[] src, uint offset = 0u, uint len = 1u)
		{
			uint num = len;
			if (offset + num > src.Length)
			{
				num = (uint)src.Length - offset;
			}
			byte[] array = new byte[len];
			Array.Copy(src, offset, array, 0L, num);
			return array;
		}
		...
		public static byte[] ConvertToRGBA(this byte[] src)
		{
			if (src == null)
			{
				return src;
			}
			byte[] src2 = src.Take(1024).ToArray();
			byte[] array = src.Skip(1024).ToArray();
			byte[] array2 = new byte[array.Length * 4];
			int num = 0;
			foreach (byte b in array)
			{
				array2.SetByteArray(num, src2.GetByteArray(b * 4, 4));
				num += 4;
			}
			return array2;
		}

		public static byte[] ConvertToIndexed8(this uint[] src, uint[] colors)
		{
			if (src == null || colors == null)
			{
				return null;
			}
			List<uint> colorTable = new List<uint>(colors);
			return src.Select((uint x) => (byte)colorTable.IndexOf(x)).ToArray();
		}

		public static IEnumerable<byte[]> Split(this byte[] src, int size)
		{
			for (int i = 0; (float)i < (float)src.Length / (float)size; i++)
			{
				yield return src.Skip(i * size).Take(size).ToArray();
			}
		}
	}
}
